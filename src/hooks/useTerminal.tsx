  ...
  const executeCommand = (commandString: string, setInput?: (val: string) => void) => {
    ...
    switch (cmd) {
      ...
      case 'projects':
        if (args[1]) {
          ...
          if (project) {
            ...
            output = <div className="mt-2 mb-4 text-red-400">Project not found: {args.slice(1).join(' ')}</div>;
          }
        } else {
          output = (
      ...
      case 'ui':
        ...
        return;
      default:
        output = <div className="mt-2 mb-4 text-red-400">Command not found: {cmd}. Type 'help' for a list of commands.</div>;
    }
